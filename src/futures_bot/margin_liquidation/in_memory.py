from __future__ import annotations

from datetime import datetime

from futures_bot.domain.ids import (
    MarginLiquidationPolicyId,
    MarginLiquidationRuleSnapshotId,
)
from futures_bot.domain.margin_liquidation import (
    MarginLiquidationPolicy,
    MarginLiquidationRuleSnapshot,
    MarginMode,
)
from futures_bot.domain.time import ensure_aware_utc


class InMemoryMarginLiquidationRuleSnapshotStore:
    """Deterministic margin/liquidation rule snapshot store test double."""

    def __init__(self) -> None:
        self._snapshots_by_id: dict[str, MarginLiquidationRuleSnapshot] = {}
        self._snapshot_ids_by_scope: dict[tuple[str, str | None, str], set[str]] = {}

    def put(self, snapshot: MarginLiquidationRuleSnapshot) -> None:
        if snapshot.snapshot_id is None:
            raise ValueError("margin/liquidation snapshot must have snapshot_id")
        key = str(snapshot.snapshot_id)
        existing = self._snapshots_by_id.get(key)
        if existing is not None:
            if existing != snapshot:
                raise ValueError("margin/liquidation snapshot id collision")
            return
        self._snapshots_by_id[key] = snapshot
        scope = (snapshot.venue_id, snapshot.instrument_id, snapshot.margin_mode.value)
        self._snapshot_ids_by_scope.setdefault(scope, set()).add(key)

    def get(
        self,
        snapshot_id: MarginLiquidationRuleSnapshotId,
    ) -> MarginLiquidationRuleSnapshot | None:
        return self._snapshots_by_id.get(str(snapshot_id))

    def latest_for_scope(
        self,
        venue_id: str,
        instrument_id: str | None,
        margin_mode: MarginMode | str,
        checked_at: datetime,
    ) -> MarginLiquidationRuleSnapshot | None:
        checked_at = ensure_aware_utc(checked_at)
        mode = margin_mode if isinstance(margin_mode, MarginMode) else MarginMode(margin_mode)
        scope = (venue_id, instrument_id, mode.value)
        snapshot_ids = self._snapshot_ids_by_scope.get(scope, set())
        snapshots = tuple(
            self._snapshots_by_id[snapshot_id]
            for snapshot_id in snapshot_ids
            if self._snapshots_by_id[snapshot_id].captured_at <= checked_at
        )
        if not snapshots:
            return None
        return max(snapshots, key=lambda item: (item.captured_at, str(item.snapshot_id)))

    def list_snapshots(self) -> tuple[MarginLiquidationRuleSnapshot, ...]:
        return tuple(
            self._snapshots_by_id[key] for key in sorted(self._snapshots_by_id)
        )


class InMemoryMarginLiquidationPolicyStore:
    """Deterministic margin/liquidation policy store test double."""

    def __init__(self) -> None:
        self._policies_by_id: dict[str, MarginLiquidationPolicy] = {}

    def put(self, policy: MarginLiquidationPolicy) -> None:
        if policy.policy_id is None:
            raise ValueError("margin/liquidation policy must have policy_id")
        key = str(policy.policy_id)
        existing = self._policies_by_id.get(key)
        if existing is not None:
            if existing != policy:
                raise ValueError("margin/liquidation policy id collision")
            return
        self._policies_by_id[key] = policy

    def get(self, policy_id: MarginLiquidationPolicyId) -> MarginLiquidationPolicy | None:
        return self._policies_by_id.get(str(policy_id))

    def list_policies(self) -> tuple[MarginLiquidationPolicy, ...]:
        return tuple(self._policies_by_id[key] for key in sorted(self._policies_by_id))
